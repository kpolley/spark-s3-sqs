...
class SqsClient(sourceOptions: SqsSourceOptions,
  ...
  private val sqsFetchIntervalMilliseconds = sourceOptions.sqsFetchIntervalMilliseconds
  ...
  sqsScheduler.scheduleWithFixedDelay(
    sqsFetchMessagesThread, 0, sqsFetchIntervalMilliseconds, TimeUnit.MILLISECONDS
  )
  ...
  private def createSqsClient(): AmazonSQS = {
    try {
      ...
      if (!isClusterOnEc2Role) {
        val accessKey = Option(hadoopConf.getTrimmed("fs.s3a.access.key")).getOrElse("")
        val secretAccessKey = Option(hadoopConf.getTrimmed("fs.s3a.secret.key")).getOrElse("")
        val sessionToken = Option(hadoopConf.getTrimmed("fs.s3a.session.token")).getOrElse("")
        if (sessionToken != "") {
          logInfo("Using session credentials from keys provided")
          logInfo(sessionToken)
          val sessionCredentials = new BasicAWSSessionCredentialsProvider(
          ...
          val basicCredentials = new BasicAWSCredentialsProvider(
          ...
          logInfo("Trying with no credentials provided")
          AmazonSQSClientBuilder
          .standard()
          .withClientConfiguration(new ClientConfiguration().withMaxConnections(maxConnections))
          .withRegion(region)
